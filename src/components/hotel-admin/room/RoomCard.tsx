...
export function RoomCard({ room, onEditStatus, onEditDetails }: RoomCardProps) {
  const baseRate = room.basePrice ?? room.roomType?.basePrice;
  const hasAdjustedRate =
    room.effectivePrice != null &&
    Math.round(Number(room.effectivePrice)) !== Math.round(Number(baseRate));
                ...
                <>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-muted-foreground line-through opacity-70">
                      {formatCurrency(Number(baseRate ?? 0))}
                    </span>
                  ...
                  {room.pricingReason && (
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className="bg-[#C9973A]/10 text-[#C9973A] text-[9px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider">
                        {room.pricingType === "override"
                          ? "Special Rate"
                          : room.pricingType === "promotion"
                            ? "Promo Applied"
                            : room.pricingType === "seasonal"
                              ? "Seasonal"
                              : "Adjusted"}
                      </span>
