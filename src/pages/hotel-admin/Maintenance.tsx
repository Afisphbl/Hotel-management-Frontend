...
export function AdminMaintenance() {
  ...
  const handleAssign = async () => {
    ...
    try {
      setAssigning(true);
      await api.post(`hotel/maintenance/${assignTarget.id}/assign`, {
            ...
            <CardContent className='p-6'>
              <div className='flex items-center justify-between'>
                <div>
                  ...
                  <h3 className={`text-2xl font-bold ${s.color} mt-1`}>
                    {s.value}
                  </h3>
                </div>
      ...
      {assignTarget && (
        ...
        >
          ...
          >
            ...
            <div className='p-5 space-y-4'>
              <p className='text-sm text-muted-foreground'>
                Select a staff member to handle the maintenance request for Room{" "}
                {assignTarget.room?.roomNumber || assignTarget.roomId}
              </p>
              <div className='space-y-1.5'>
                ...
                >
                  ...
                  {staffList.map((st) => (
                    <option key={st.id} value={st.id}>
                      {st.firstName} {st.lastName} ({st.role?.replace("_", " ")}
                      )
                    </option>
                  ))}
                </select>
      ...
      {resolveTarget && (
        ...
        >
          ...
          >
            ...
            <div className='p-5 space-y-4'>
              <p className='text-sm text-muted-foreground'>
                Resolve maintenance ticket for Room{" "}
                {resolveTarget.room?.roomNumber || resolveTarget.roomId}
              </p>
              <div className='space-y-1.5'>
              ...
              <div className='flex gap-3 pt-2'>
                ...
                <Button
                  ...
                  onClick={() =>
                    handleAction(resolveTarget.id, "resolve", {
                      notes: resolveNotes,
                      cost: resolveCost ? parseFloat(resolveCost) : undefined,
                    })
                  }
                >
                  Resolve
