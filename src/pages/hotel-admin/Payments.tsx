...
import {
  ...
  Plus, ReceiptText, User, Building, CreditCard, ArrowLeftRight, RotateCcw,
} from 'lucide-react';
...
export function AdminPayments() {
  ...
  const [refundForm, setRefundForm] = useState({ amount: '', reason: 'cancellation', notes: '' });

  const PAGE_SIZE = 15;

  useEffect(() => { setPage(1); }, [filterStatus]);
  useEffect(() => { fetchPayments(); fetchSummary(); }, [page, filterStatus]);
  ...
  const fetchSummary = async () => {
    try {
      ...
      console.log('[Payments] summaryData:', data);
      setSummaryData(data);
    } catch {
  ...
  const openDetail = async (payment: any) => {
    ...
    try {
      const [detailRes, refundsRes] = await Promise.all([
        ...
        api.get(`finance/refunds/by-payment/${payment.id}`).catch(() => ({ data: [] })),
      ]);
  ...
  const submitCreatePayment = async () => {
    ...
    try {
      ...
      setCreateForm({ invoiceId: '', amount: '', method: 'cash', transactionId: '', description: '' });
      fetchPayments();
      fetchSummary();
    } catch (err: any) {
  ...
  const submitRefund = async () => {
    ...
    try {
      ...
      setRefundForm({ amount: '', reason: 'cancellation', notes: '' });
      const refundsRes = await api.get(`finance/refunds/by-payment/${activePayment.id}`).catch(() => ({ data: [] }));
      setActiveRefunds(refundsRes.data || []);
      fetchPayments();
      fetchSummary();
    } catch (err: any) {
  ...
  const totalRevenue = summaryData.filter(p => p.status === 'completed').reduce((sum, p) => sum + (p.amount || 0), 0);
  ...
  return (
    <div className="space-y-8 pb-10">
      <div className="flex items-center justify-between">
        ...
        <Button className="bg-[#0F1B2D] hover:bg-[#1a2a3a]" onClick={() => setIsCreateOpen(true)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl font-bold text-[#0F1B2D] mt-1">{summaryData.length}</h3>
              </div>
        ...
        <Card className="shadow-sm border-none bg-white">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-medium text-muted-foreground uppercase">Refunded</p>
                <h3 className="text-2xl font-bold text-purple-600 mt-1">
                  {summaryData.filter(p => p.status === 'refunded' || p.status === 'partially_refunded').length}
                </h3>
              </div>
      ...
      <Sheet open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <SheetContent className="w-full sm:max-w-xl overflow-y-auto">
          <SheetHeader className="border-b pb-4">
            <SheetTitle className="text-xl font-serif">Record Payment</SheetTitle>
            <SheetDescription>Enter payment details to record a new transaction</SheetDescription>
          </SheetHeader>

          <div className="space-y-4 p-4">
            <div className="space-y-2">
              <label className="text-sm font-medium text-[#0F1B2D]">Invoice ID</label>
              <Input
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-[#0F1B2D]">Method</label>
                <Select value={createForm.method} onValueChange={(v) => setCreateForm({ ...createForm, method: v })}>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-[#0F1B2D]">Transaction ID</label>
              <Input
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-[#0F1B2D]">Description</label>
              <textarea
      ...
      <Sheet open={isDetailOpen} onOpenChange={setIsDetailOpen}>
        <SheetContent className="w-full sm:max-w-3xl overflow-y-auto">
          <SheetHeader className="border-b pb-4">
            <SheetTitle className="text-xl font-serif">Payment Details</SheetTitle>
            <SheetDescription>View payment information and manage refunds</SheetDescription>
          </SheetHeader>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Card className="border-none bg-gray-50">
                  <CardContent className="p-4 space-y-1">
                    ...
                    <p className="text-xl font-bold">{formatCurrency(Number(activePayment.amount))}</p>
                    {activePayment.fee > 0 && (
                      <p className="text-xs text-muted-foreground">Fee: {formatCurrency(Number(activePayment.fee))} | Net: {formatCurrency(Number(activePayment.netAmount))}</p>
                    )}
                  </CardContent>
                </Card>
                <Card className="border-none bg-gray-50">
                  <CardContent className="p-4 space-y-1">
                    ...
                    <Badge className={cn('text-xs', STATUS_STYLES[activePayment.status])}>{activePayment.status}</Badge>
                    {activePayment.paidAt && (
                      <p className="text-xs text-muted-foreground">Paid: {new Date(activePayment.paidAt).toLocaleString()}</p>
                    )}
                  </CardContent>
                </Card>
                <Card className="border-none bg-gray-50">
                  <CardContent className="p-4 space-y-1">
                    <p className="text-xs text-muted-foreground uppercase font-medium">Method</p>
                    <p className="text-sm capitalize font-medium flex items-center gap-1">
                      <CreditCard className="w-3.5 h-3.5" /> {activePayment.method?.replace(/_/g, ' ')}
                    </p>
                    {activePayment.currency && <p className="text-xs text-muted-foreground">{activePayment.currency}</p>}
                  </CardContent>
                </Card>
                <Card className="border-none bg-gray-50">
                  <CardContent className="p-4 space-y-1">
                    <p className="text-xs text-muted-foreground uppercase font-medium">Transaction ID</p>
                    <p className="text-sm font-mono">{activePayment.transactionId || '—'}</p>
                    {activePayment.id && <p className="text-xs text-muted-foreground">ID: {activePayment.id.slice(0, 12)}...</p>}
                  </CardContent>
                </Card>
              </div>
                  ...
                  <Card className="border-none bg-gray-50">
                    <CardContent className="p-4 space-y-1">
                      <div className="flex justify-between">
                        ...
                        <span className="text-sm font-mono">{activePayment.invoice.id?.slice(0, 12)}...</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-xs text-muted-foreground">Invoice Amount</span>
                        <span className="text-sm font-medium">{formatCurrency(Number(activePayment.invoice.amount))}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-xs text-muted-foreground">Invoice Status</span>
                        <Badge className="text-xs">{activePayment.invoice.status}</Badge>
                      </div>
                      {activePayment.invoice.invoiceNumber && (
                        <div className="flex justify-between">
                          <span className="text-xs text-muted-foreground">Invoice #</span>
                          <span className="text-sm">{activePayment.invoice.invoiceNumber}</span>
                        </div>
                      )}
                    </CardContent>
                  </Card>
                  ...
                  <Card className="border-none bg-gray-50">
                    <CardContent className="p-4 space-y-1">
                      <div className="flex justify-between">
                        ...
                        <span className="text-sm font-mono">{activePayment.booking.id?.slice(0, 12)}...</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-xs text-muted-foreground">Check-in</span>
                        <span className="text-sm">{activePayment.booking.checkIn ? new Date(activePayment.booking.checkIn).toLocaleDateString() : '—'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-xs text-muted-foreground">Check-out</span>
                        <span className="text-sm">{activePayment.booking.checkOut ? new Date(activePayment.booking.checkOut).toLocaleDateString() : '—'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-xs text-muted-foreground">Total Price</span>
                        <span className="text-sm font-medium">{formatCurrency(Number(activePayment.booking.totalPrice))}</span>
                      </div>
                      {activePayment.booking.guest && (
                        <div className="flex justify-between items-center pt-1 border-t border-gray-200 mt-1">
                          <span className="text-xs text-muted-foreground flex items-center gap-1"><User className="w-3 h-3" /> Guest</span>
                          <span className="text-sm">{activePayment.booking.guest.firstName} {activePayment.booking.guest.lastName}</span>
                        </div>
                      )}
                    </CardContent>
                  </Card>
                      ...
                      <Card key={r.id} className="border-none bg-red-50">
                        <CardContent className="p-3 flex justify-between items-center">
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground capitalize">{r.reason} {r.processedAt ? `• ${new Date(r.processedAt).toLocaleDateString()}` : ''}</p>
                            {r.notes && <p className="text-xs text-muted-foreground mt-0.5">{r.notes}</p>}
                          </div>
                          <Badge className={cn('text-xs', r.status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800')}>
                            {r.status}
                          </Badge>
                        </CardContent>
                      </Card>
                  ...
                  <CardContent className="p-4 space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <label className="text-xs font-medium text-[#0F1B2D]">Amount</label>
                        <Input
                          type="number" min="0" step="0.01"
                      ...
                      <div className="space-y-1">
                        <label className="text-xs font-medium text-[#0F1B2D]">Reason</label>
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-medium text-[#0F1B2D]">Notes</label>
                      <Input
