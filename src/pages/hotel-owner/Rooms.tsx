...
export function RoomsPage() {
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      setUpdating(true);
      await api.patch(`hotel/rooms/${editingRoom.id}/status`, {
        status: newStatus,
      });
      setRooms((prev) =>
        prev.map((r) =>
          r.id === editingRoom.id ? { ...r, status: newStatus } : r,
        ),
      );
      setEditingRoom(null);
      fetchSummary();
        ...
        <CardContent className='p-6'>
          <div className='flex flex-col sm:flex-row gap-4'>
            ...
            <div className='flex gap-2'>
              ...
              <Select
                value={sortBy}
                onValueChange={(v) => setSortBy(v || "roomNumber")}
              >
                <SelectTrigger className='w-[160px] bg-white'>
...
function RoomStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={cn("px-3 py-1 rounded-full text-xs font-medium", c.bg, c.text)}
    >
      {c.label}
    </span>
  );
