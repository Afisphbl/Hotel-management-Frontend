  ...
  return (
    <div className="max-w-4xl space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl sm:text-3xl font-serif text-[#0F1B2D]">Property Settings</h2>
      ...
      <div className="flex flex-col md:flex-row gap-6">
        <div className="w-full md:w-64 space-y-1 overflow-x-auto flex md:flex-col pb-2 md:pb-0 no-scrollbar">
          {[
            ...
            { label: 'Booking', icon: ShieldCheck },
            ...
            <button key={i} className={cn(
                ...
                item.active ? "bg-[#0F1B2D] text-white shadow-sm" : "hover:bg-slate-100/50 text-slate-500"
            )}>
                <div className="flex items-center gap-3">
                   <item.icon className="w-4 h-4" />
                   <span className="text-sm font-medium">{item.label}</span>
                </div>
                {!item.active && <ChevronRight className="hidden md:block w-4 h-4 opacity-0 group-hover:opacity-100" />}
            </button>
          ))}
        </div>
          ...
          <Card className="shadow-sm border-none bg-white">
            <CardHeader className="flex flex-col sm:flex-row sm:items-center gap-4">
               ...
               <div>
                  ...
                  <CardDescription>Managed by Peninsula Group Ltd.</CardDescription>
            ...
            <CardContent className="space-y-6 pt-4 border-t border-slate-50">
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                     ...
                     <Input defaultValue="Grand Peninsula Resort" />
                  </div>
                  <div className="space-y-2">
               ...
               <div className="space-y-2">
                  ...
                  <Input defaultValue="Elegance redefined on the Adriatic coast." />
               </div>

               <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                     <Label>Support Phone</Label>
                     <Input defaultValue="+382 67 123 456" />
                  </div>
                  <div className="space-y-2">
                     <Label>Official Website</Label>
                     <Input defaultValue="https://grandpeninsula.com" />
                  </div>
               </div>
            </CardContent>
          </Card>
