...
export function PlatformSubscriptions() {
  ...
  const filteredPlans = useMemo(() => {
    ...
    return plans.filter((p: any) => {
      ...
      const matchesPlan =
        planFilter === "all" || (p.plan || "").toUpperCase() === planFilter;
    ...
    try {
      await updateMutation.mutateAsync({
        ...
        data: {
          ...
          features: {
            enabledFeatures: editingPlan.features
              .split(",")
              .map((f: string) => f.trim())
              .filter(Boolean),
          },
  ...
  return (
    <div className='space-y-6'>
      <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-4'>
        <div>
          ...
          <p className='text-sm text-muted-foreground mt-1'>
            Manage tiers and pricing models.
          </p>
        </div>
        <Button
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
        {plansLoading ? (
          ...
          <Card className='md:col-span-3 shadow-sm border-none bg-white'>
            <CardContent className='p-12 text-center text-muted-foreground'>
              ...
              <p className='text-xs text-slate-400 mt-1'>
                The data might not be available from the server.
              </p>
            </CardContent>
          </Card>
          ...
          filteredPlans.map((plan: any) => (
            <Card
              key={plan.id}
              className='shadow-sm border-none bg-white relative overflow-hidden group flex flex-col'
            >
              ...
              <CardHeader>
                <div className='flex justify-between items-start'>
                  <div>
                    ...
                    <CardDescription>
                      {plan.hotels ?? plan.activeProperties ?? 0} Active
                      Properties
                    </CardDescription>
                  </div>
      ...
      <Card className='shadow-sm border-none bg-white'>
        <CardHeader>
          <CardTitle className='font-serif text-xl'>
            Top Subscribing Properties
          </CardTitle>
          <CardDescription>
            A list of hotels by their revenue contribution.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
              ...
              {topLoading ? (
                ...
                topHotels.map((hotel: any) => (
                  <TableRow key={hotel.id}>
                    <TableCell className='font-medium text-[#0F1B2D]'>
                      {hotel.name}
                    </TableCell>
                    <TableCell className='hidden sm:table-cell'>
                      <Badge
          ...
          <div className='space-y-4 py-4'>
            <div className='space-y-2'>
              ...
              <select
                ...
                value={newTier.plan}
                onChange={(e) =>
                  setNewTier({ ...newTier, plan: e.target.value })
                }
              >
            ...
            <div className='space-y-2'>
              ...
              <Input
                ...
                value={newTier.price}
                onChange={(e) =>
                  setNewTier({ ...newTier, price: Number(e.target.value) })
                }
              />
            </div>
            <div className='space-y-2'>
