...
function StatCard({
  ...
  return (
    <div className='bg-[#F8F7F4] p-4 rounded-xl group hover:bg-[#F0EEE6] transition-colors'>
      <div className='flex items-center justify-between mb-2'>
        <p className='text-[10px] text-muted-foreground uppercase font-bold tracking-widest'>
          {label}
        </p>
        <Icon className='w-3.5 h-3.5 text-[#C9973A] opacity-50 group-hover:opacity-100 transition-opacity' />
      </div>
      {value != null ? (
        <p className='text-2xl font-serif text-[#0F1B2D]'>{value}</p>
      ) : (
...
export function HotelOverview() {
  ...
  const { data: allLogs } = usePlatformAuditLogs();

  // Filter logs for this specific hotel if hotel name or id matches (backend logs might use name or id)
  const hotelLogs =
    allLogs?.filter(
      (log: any) =>
        log.hotelId === id ||
        (hotel?.name && log.hotel?.toLowerCase() === hotel.name.toLowerCase()),
    ) || [];
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
          ...
          <CardContent className='space-y-8'>
            <div className='grid grid-cols-2 md:grid-cols-3 gap-6'>
              ...
              <DataField
                ...
                value={
                  hotel.created
                    ? format(new Date(hotel.created), "MMMM d, yyyy")
                    : null
                }
              />
              <div>
            ...
            <div className='grid grid-cols-2 md:grid-cols-4 gap-4 pt-6 border-t border-muted'>
              <StatCard label='Rooms' value={hotel.totalRooms} icon={Bed} />
              <StatCard
                label='Occupancy'
                value={hotel.currentOccupancy}
                icon={Users}
              />
              <StatCard
                label='Storage'
                value={hotel.storageUsed}
                icon={HardDrive}
              />
              <StatCard
                label='Location'
        ...
        <Card className='shadow-sm border-none bg-white'>
          ...
          <CardContent className='space-y-4'>
            {hotelLogs.length > 0 ? (
              hotelLogs.slice(0, 5).map((log: any, i: number) => (
              ...
              <div className='flex flex-col items-center justify-center py-10 text-center border border-dashed border-slate-100 rounded-xl'>
                <Clock className='w-8 h-8 text-slate-200 mb-2' />
                <p className='text-[11px] font-bold text-slate-400 uppercase tracking-widest'>
                  Live Activity Stream
                </p>
                <p className='text-[10px] text-slate-300 mt-1 max-w-[150px]'>
                  Hotel-specific audit logging will be implemented soon.
                </p>
              </div>
            )}
          </CardContent>
